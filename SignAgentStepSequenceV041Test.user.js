// ==UserScript==
// @name         SignAgent Step Sequence v0.4.1 TEST
// @namespace    signbrothers-tools
// @version      0.4.1
// @description  Test build: {start:step}, {seq}, {seqv}, and combined multi-field sequencing with stable helper rendering.
...
(() => {
  ...
  function renderCombined(form) {
    ...
    const signature = [
      parseIds(form).join(','),
      ...helpers.map(h => {
        const input = helperInput(h);
        const mode = isSeqv(input?.value) ? 'seqv' : isSeq(input?.value) ? 'seq' : 'step';
        return `${input?.name || input?.id || 'field'}:${mode}`;
      })
    ].join('|');
  ...
  function renderAll(form) {
    ...
    form.querySelectorAll(WRITABLE).forEach(input => {
      if (input.closest(`.${HELPER}`) || input.closest(`.${COMBINED}`)) return;
      render(input, ids);
    });
    renderCombined(form);
  }
